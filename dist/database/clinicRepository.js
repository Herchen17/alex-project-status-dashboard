...
class ClinicRepository {
    constructor(dbConnection) {
        this.db = dbConnection || (0, connection_1.getDatabase)();
    }
    /**
    ...
    async create(clinicData) {
        ...
        `, [
            ...
            const dbError = new Error(`Failed to create clinic: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async findById(id) {
        try {
            ...
            const clinicRow = await this.db.get(`
      ...
      `, [id]);
            ...
            const dbError = new Error(`Failed to find clinic by ID: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async findAll(filters) {
        try {
            ...
            const dbError = new Error(`Failed to find clinics: ${error instanceof Error ? error.message : String(error)}`);
            dbError.code = 'READ_ERROR';
            throw dbError;
        }
    }
    ...
    async update(id, updateData) {
        try {
            ...
            const dbError = new Error(`Failed to update clinic: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async delete(id) {
        try {
            ...
            const dbError = new Error(`Failed to delete clinic: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async count(filters) {
        try {
            ...
            const dbError = new Error(`Failed to count clinics: ${error instanceof Error ? error.message : String(error)}`);
            dbError.code = 'COUNT_ERROR';
            throw dbError;
        }
    }
    // Private helper methods
    async buildClinicObject(row) {
        // Fetch specialties
        const specialtyRows = await this.db.all(`
      SELECT s.name 
      FROM specialties s 
    ...
    async insertSpecialties(clinicId, specialties) {
        for (const specialtyName of specialties) {
            // Find or create specialty
            let specialty = await this.db.get('SELECT id FROM specialties WHERE name = ?', [specialtyName]);
            if (!specialty) {
            ...
            await this.db.run('INSERT OR IGNORE INTO clinic_specialties (clinic_id, specialty_id) VALUES (?, ?)', [clinicId, specialty.id]);
        }
    }
    async insertDecisionMakers(clinicId, decisionMakers) {
    ...
    async insertOperatingHours(clinicId, operatingHours) {
    ...
    async updateSpecialties(clinicId, specialties) {
    ...
    async updateDecisionMakers(clinicId, decisionMakers) {
    ...
    async updateOperatingHours(clinicId, operatingHours) {
...
//# sourceMappingURL=clinicRepository.js.map