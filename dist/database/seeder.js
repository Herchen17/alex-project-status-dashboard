...
class DatabaseSeeder {
    ...
    async initializeSchema() {
      ...
      `);
            ...
            const dbError = new Error(`Schema initialization failed: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async seedSpecialties() {
        try {
            ...
            for (const [name, category] of specialties) {
                try {
                    await this.db.run('INSERT OR IGNORE INTO specialties (name, category) VALUES (?, ?)', [name, category]);
                }
                catch (error) {
            ...
            const dbError = new Error(`Specialties seeding failed: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async dropTables() {
        try {
            ...
            const dbError = new Error(`Table drop failed: ${error instanceof Error ? error.message : String(error)}`);
